...
class Jx3ApiPlugin(Star):
    ...
    def load_local_base64(self):
        """加载图片文件的base64编码"""
        img = self._safe_load_base64_dir(self.plugin_temp_img, "img")
        sect = self._safe_load_base64_dir(self.plugin_temp_sect, "sect")
        serendipity = self._safe_load_base64_dir(self.plugin_temp_serendipity, "serendipity")
        self.icons =  {
            "img": img,
            "sect": sect,
        ...
        logger.debug(
            "图片base64编码加载完成: img=%s, sect=%s, serendipity=%s",
            len(img),
            len(sect),
            len(serendipity),
        )
    ...
    def _safe_load_base64_dir(self, dir_path: Path, label: str) -> dict[str, str]:
        """加载指定目录的图片资源，异常时返回空字典避免渲染链路中断。"""
        try:
            if not dir_path.exists() or not dir_path.is_dir():
                logger.warning(f"图片目录不存在或不可用: {dir_path}")
                return {}
            # 跳过空目录，减少无意义的解码工作
            if not any(os.scandir(dir_path)):
                logger.warning(f"图片目录为空: {dir_path}")
                return {}
            return load_as_base64(str(dir_path))
        except Exception as e:
            logger.exception(f"加载{label}图片资源失败: {e}")
            return {}
